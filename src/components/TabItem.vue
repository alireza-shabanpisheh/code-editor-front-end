<template>
  <div 
    class="tab-item flex items-center px-3 py-2 border-r border-gray-700 cursor-pointer min-w-0 max-w-xs group"
    :class="tabClasses"
    @click="$emit('click', file.id)"
  >
    <!-- آیکون فایل -->
    <div 
      class="w-3 h-3 rounded-sm flex items-center justify-center text-xs font-bold mr-2 flex-shrink-0"
    ...
    <div 
      v-if="file.isDirty"
      class="w-2 h-2 bg-orange-500 rounded-full ml-1 flex-shrink-0"
