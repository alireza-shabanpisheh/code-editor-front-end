<template>
  <div class="file-tree-node">
    <div class="flex items-center py-1 px-2 hover:bg-gray-700 cursor-pointer rounded text-sm group"
      :class="{ 'bg-gray-700': isSelected }" :style="{ paddingLeft: `${(level * 16) + 8}px` }" @click="handleClick"
      ...
      <div class="w-4 h-4 mr-2 flex items-center justify-center">
        ...
        <template v-else>
          <div class="w-3 h-3 rounded-sm flex items-center justify-center text-xs font-bold"
            :class="getFileIconClassForNode()">
            {{ getFileIconTextForNode() }}
          </div>
        </template>
      </div>

      <span class="truncate flex-1" :class="getFileNameClass()">
        {{ node.name }}
      </span>
      ...
      <div class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center space-x-1 ml-2">
        <template v-if="node.type === 'folder'">
          <button @click.stop="$emit('create-file', node.id)" class="p-1 hover:bg-gray-600 rounded text-xs"
            title="New File">
            📄
          </button>
          <button @click.stop="$emit('create-folder', node.id)" class="p-1 hover:bg-gray-600 rounded text-xs"
            title="New Folder">
            📁
          </button>
        </template>
        <button @click.stop="$emit('delete-item', node.id)" class="p-1 hover:bg-gray-600 rounded text-xs text-red-400"
          title="Delete">
          🗑️
        </button>
      </div>
...
function getFileIconClassForNode() {
  return getFileIconClass(props.node.fileType)
}

function getFileIconTextForNode() {
  return getFileIconText(props.node.fileType)
}
